...
- JSON 包装订阅可解析 `list`、`records`、`rows`、`entries`、`node`/`nodes`、`proxy`/`proxies`、`server`/`servers`、`nodeList`、`proxyList`、`serverList`、`subscriptionList`、`urlList`、`linkList` 及其 snake_case/kebab-case 变体等常见集合字段，容器名不会污染缺少 fragment 的节点 URI 展示名。
