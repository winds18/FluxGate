...
- Clash YAML Hysteria/Hysteria2 结构化节点会保留 `fp`、`client-fingerprint`、`client_fingerprint` 和 `clientFingerprint` uTLS 指纹别名；Hysteria v1 的 `disable-sni`/`disable_sni` 也会同步为 URI `disable_sni=1`。
