...
- 管理后台概览页已把真实测试闭环升级为初始化向导，按来源、节点池、虚拟网关、团队 Token 和访问策略展示准备状态，同时提供可测进度、当前建议动作、可点击模块入口和每步提示文案；每个闭环步骤都有序号、符号化状态单元和就绪/待补徽标，浏览器 QA 会校验进度、当前动作、5 条提示、序号、符号、状态徽标和无视觉溢出。
