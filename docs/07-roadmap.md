...
- subscription 来源可解析 V2Ray/Xray JSON `outbounds` 中的 VMess、VLESS、Trojan、Shadowsocks、HTTP、SOCKS、freedom 和 blackhole 出站，并保留常见 TLS、REALITY、WebSocket、gRPC、HTTP/H2 和 HTTPUpgrade 传输参数。
- 节点来源前缀和展示名规则。
- 标签管理。
- 虚拟节点管理。
- 策略管理基础列表和创建。
