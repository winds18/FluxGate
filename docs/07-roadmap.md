...
- SQLite migration。
- 嵌入式管理后台只读仪表盘。
- 管理后台已切换为模块化控制台布局，包含带模块符号的桌面侧边导航、移动底部 Dock、移动“更多”菜单、概览/接入/节点/身份/策略/流量/运维视图，并纳入浏览器导航、符号数量和溢出验收。
- 管理后台工作区标题已补齐当前模块符号，与侧边导航和移动 Dock 统一，并纳入浏览器逐模块切换验收。
- 管理后台桌面侧边导航、移动 Dock 和移动“更多”菜单已补齐模块数量徽标，把来源、节点、Token、策略、流量样本和配置就绪状态前置到导航层，并纳入浏览器数量、填充值和移动端溢出验收。
- 管理后台移动端 Dock 已按 A 方案收敛为概览、接入、节点、身份和“更多”5 个入口，策略、流量和运维放入“更多”菜单；菜单选择后自动收起，当前处于低频模块时“更多”保持高亮，并纳入移动浏览器交互验收。
- 管理后台创建/导入表单默认收纳为模块抽屉，抽屉标题、展开/收起控件、提交控件、登录入口和顶部退出/刷新控件补齐统一功能符号，抽屉默认占满模块操作区宽度，减少首屏视觉负担，并纳入浏览器符号顺序、按钮符号、宽度和溢出验收。
- 管理后台创建/导入表单展开后已改为真正的右侧侧滑工作台，同一时间只保留一个打开，桌面端使用稳定窄宽和轻量背景承托，移动端避开底部 Dock；`Escape` 收起侧滑层但保留草稿，提交成功后自动回到列表，退出登录会清理页面外壳状态，并纳入浏览器 dialog 语义、草稿保留、提交后收起、无溢出和状态恢复验收。
- 管理后台顶部刷新已补齐人性化等待态，点击后按钮显示 `刷新中`、禁用重复点击并同步主工作区 `aria-busy`，完成后恢复原符号和标签；该链路已纳入浏览器请求拦截验收。
- 管理后台顶部退出已补齐人性化等待态，点击后按钮显示 `退出中`、禁用重复点击并同步主工作区 `aria-busy`；完成后干净回到登录页，该链路已纳入浏览器请求拦截验收。
...
- 管理后台概览页已把真实测试闭环看板升级为 7 步初始化向导，可直接查看来源、节点池、虚拟网关、团队 Token、订阅分发、访问策略和发布检查是否齐备；向导会加载交付收口接口，展示可测进度、当前建议动作、模块入口和每步提示，概览快速定位条也同步为“初始化”标签和向导符号；每个闭环步骤都有序号、符号化状态单元和就绪/待补徽标，并纳入浏览器进度、当前动作、提示文案、定位条标签/符号、状态徽标和溢出验收。
- 管理后台概览页首屏已加入任务总控条，直接回答“现在能不能真实测试”和“下一步点哪里”，并把订阅、节点和网关三类核心状态收敛到稳定状态单元；主操作会深链到待补表单或发布配置按钮，并纳入静态契约与浏览器跳转/溢出验收。
- 管理后台概览页初始化向导已加入真实测试动作区，把“去复制订阅”“检查收口”“发布配置”三步并排呈现，分别深链到 Token 列表、收口检查按钮和发布配置按钮，并纳入静态契约与浏览器定位反馈验收。
- 管理后台概览页快速入口卡已加入模块符号、实时数量和就绪徽标，来源、地区、Token 和发布状态在概览首屏可直接扫描，并纳入浏览器符号、数量和溢出验收。
- 管理后台概览页快速入口已补齐具体目标定位：从概览进入接入、节点、身份和运维时会直接落到关键面板；模块标题摘要与面板入口改为横向可扫命令条，降低首屏拥挤和反复查找成本。
- 管理后台概览页下一步入口已改为结构化动作卡，目标模块符号、状态说明和统一符号主操作按钮分层展示，并纳入浏览器按钮符号与溢出验收。
...
- 管理后台模块标题区已补齐随视图变化的顶部主操作，概览按初始化进度给出下一步，其他模块可直达添加来源、创建网关、签发 Token、创建策略或运维入口，并纳入浏览器标签、符号、跳转和溢出验收。
- 管理后台模块跳转动作已补齐落点反馈，顶部主操作、快速定位和空状态动作会在目标面板/抽屉上短暂高亮，并在表单展开后自动聚焦第一个可输入字段，减少点击后重新找入口的成本。
- 管理后台模块跳转动作已补齐持续定位反馈，顶部状态会显示 `已定位：目标面板`，模块快速定位条会保留当前落点高亮和 `aria-current`，减少点击后需要重新判断位置的成本，并纳入浏览器交互验收。
- 管理后台表单必填项已补齐统一状态反馈，缺少必填字段时会提示 `请先补齐：字段名`，并自动高亮当前表单、聚焦待补字段，避免真实操作时只看到浏览器默认校验气泡。
- 管理后台表单必填项已补齐字段旁内联反馈，缺少必填字段时会显示 `请填写字段名`，同步可访问性错误属性，并在用户输入后自动清除提示与错误状态，减少“我知道错了但不知道改哪里”的操作摩擦。
- 管理后台创建/导入表单抽屉已补齐草稿感知，用户开始输入或变更字段后会在抽屉标题显示 `草稿未提交`，提交成功或取消后自动清除，帮助用户判断当前输入是否仍是未保存状态。
- 管理后台创建/导入表单抽屉已补齐统一取消动作，展开后可一键清空草稿、清除字段错误、收起抽屉并反馈 `已取消：目标表单`，减少误操作后找不到恢复入口的摩擦，并纳入浏览器交互与溢出验收。
- 管理后台创建/导入表单抽屉已补齐提交中反馈，提交后抽屉会进入 `aria-busy`，临时禁用输入、取消和提交控件，按钮按原动作显示 `添加中`/`创建中`/`导入中`，完成后恢复原状态，降低重复点击和不确定等待带来的摩擦。
- 管理后台表单控件已统一为稳定输入节奏，登录、侧滑抽屉、Token 操作和节点编辑中的输入框、选择框、单位输入组与按钮共享高度、内边距和字段面板背景，移动端自动回落为单列触控；静态契约和浏览器 QA 会量测高度、分栏、间距和溢出。
- 管理后台 UI 已补齐现代视觉基线层，命令中心、内容面板、概览 KPI、模块工作台和普通信息卡统一使用白色主表面、浅灰内卡片、共享边框、zashboard 式低阴影和低矮面板标题栏；静态契约和浏览器 QA 会确认 quiet elevation 令牌、标题栏高度与真实渲染样式，防止后续模块继续产生割裂层级、厚重卡片阴影或大块列表头。
- 管理后台模块工作台已进一步压到低矮控制条节奏，使用 6px 间距、24px 工作台符号和低高度内卡片；浏览器 QA 会量测逐模块真实高度，防止接入、节点、身份、Token、策略、流量和运维顶部区域重新膨胀。
- 管理后台桌面侧边栏已收敛为窄白导航 rail，桌面宽度固定 192px，导航行高、模块符号和底部提示都按 zashboard 式扫描节奏降噪；静态契约和浏览器 QA 会量测白底、宽度、行高、符号尺寸和无溢出。
- 管理后台移动端已补齐底部 Dock 安全区契约，工作区底部留白、更多菜单、表单侧滑层和节点详情抽屉统一使用 Dock 高度/间距变量；浏览器 QA 会滚动到移动端概览底部，验证最后内容块不会被 Dock 遮挡。
- 管理后台页面截图验收已升级为多视口矩阵，默认采集 1440x900、1280x720 和 390x844 三档桌面/移动截图，并由静态契约守住，便于 UI 改动后复核无遮挡和无溢出。
- 管理后台行内卡片保存已补齐人性化等待态，来源、团队、成员、虚拟网关、策略和节点保存期间会锁定当前卡片控件、显示 `保存中`、标记 `aria-busy` 并在请求完成后恢复编辑入口；来源卡片保存链路已纳入浏览器拦截验收，避免重复点击和“保存到底有没有生效”的不确定感。
- 管理后台行内卡片操作已降噪为紧凑工具条：编辑、刷新、同步命名和恢复自动等普通动作不再以满宽蓝色按钮抢占视线，保存动作继续保持明确主操作权重，移动端保留全宽触控；该层级已纳入静态 UI 契约。
- 管理后台地区节点卡片已减少动作区对扫描流的干扰，编辑和恢复自动停靠在右上角，卡片不再因两个普通按钮额外撑高，移动端仍保留大按钮触控；该规则已纳入静态 UI 契约。
- 管理后台节点卡片操作已进一步人性化为符号优先按钮，桌面端通过 `aria-label` 和悬停提示表达“编辑节点 / 恢复自动命名”，避免卡片右侧出现被压缩的“恢复...”半截文案；移动端仍显示完整文字。
...
- 管理后台身份模块顶部已新增身份分发总控工作台，先展示团队、成员、Token 和可分发订阅 4 个核心状态，并用团队归属、成员承接、订阅分发 3 个阶段卡说明分发链路是否齐备；该层级已纳入浏览器符号、阶段卡和无溢出验收。
- 管理后台上游来源列表已改为卡片式信息层级，新增类型、前缀、刷新方式和同步状态摘要芯片，编辑、刷新和同步命名动作已统一为符号按钮，并纳入视觉溢出与符号数量验收。
- 管理后台虚拟网关列表已改为卡片式信息层级，新增监听、策略、状态和标签筛选范围摘要芯片，监听信息、策略、状态和标签选择器分层展示，编辑/保存/取消动作已统一为符号按钮，并纳入动作符号和视觉溢出验收。
- 管理后台策略列表已改为卡片式信息层级，新增作用域、节点上限、状态和虚拟网关范围摘要芯片，标签限制、允许虚拟网关、最大节点数和状态分层展示，编辑/保存/取消动作已统一为符号按钮，并纳入动作符号和视觉溢出验收。
- 管理后台 Token 卡片已加入状态、归属成员、到期状态和额度状态摘要芯片，额度、订阅地址和 Token 操作小节补齐符号标题与说明胶囊，并把订阅地址拆成默认、Clash/Mihomo 和 sing-box 的可复制订阅卡，长 URL 省略在卡片内部，避免挤压操作按钮。
- 管理后台 Token 订阅卡已补齐“打开”入口，可直接预览或下载三类订阅内容；打开和复制入口已改为紧凑符号按钮，并纳入浏览器数量、符号和溢出验收。
- 管理后台 Token 订阅卡已补齐“探测”入口，可在后台直接验证默认、Mihomo 和 sing-box 订阅地址是否能返回内容，并把最近探测状态、检查时间和内容大小写回订阅卡元信息；探测动作只复用站内订阅路由，不对任意外部地址发起请求，并纳入浏览器点击反馈验收。
...
- 管理后台 Token 订阅分发工作台已补齐首屏动作入口，可直接定位订阅卡、复制 Mihomo 订阅地址或打开 Mihomo 订阅内容，减少真实测试时反复在卡片里寻找客户端地址的操作成本，并纳入静态契约与浏览器动作符号、定位反馈、跳转和无溢出验收。
- 管理后台 Token 卡片已加入额度使用率进度条，帮助在身份模块里直接扫到已用比例，并纳入浏览器数量和溢出验收。
- 管理后台 Token 续期、加额、停用、恢复和重置订阅地址已补齐卡片级等待态，操作中锁定当前卡片输入和状态控制按钮，触发按钮展示动作进行中标签，完成后恢复原状态，并纳入浏览器拦截续期请求验收。
- 管理后台订阅复制动作已加入按钮级“已复制”反馈，并纳入浏览器点击验收。
- 管理后台创建/重置 Token 后的订阅结果框已复用订阅卡布局，并补齐符号标题和格式数量胶囊，避免结果区长 URL 或说明文案造成横向溢出。
...
- 管理后台运维模块已改为带操作符号、状态胶囊的配置操作卡和结构化结果卡，配置检查结果可分层展示结果符号、状态胶囊、Hash、入站、出口、上游和用户数量；交付收口检查会汇总来源、节点、虚拟网关、Token、策略和配置是否达到真实可测状态。运维操作执行中会突出当前卡片、显示 `检查配置中` 等等待文案，并临时锁定同组高风险按钮，已纳入浏览器检查、恢复态和视觉溢出验收。
- 管理后台运维模块顶部已新增交付发布工作台，先展示闭环进度、配置 Hash、入站和上游出口 4 个核心状态，并用来源、节点、虚拟网关、Token、策略和配置 6 个步骤卡说明发布链路是否齐备；该层级已纳入浏览器符号、步骤卡和无溢出验收。
- 管理后台空状态已统一为带模块符号、标题、提示文案和下一步动作的结构化空状态，来源、节点、身份、策略、Token 和流量空态可直接跳转到对应创建、导入或运维入口；流量模块空状态已纳入浏览器动作跳转、符号与溢出验收。
...
- 管理后台节点详情已加入协议符号摘要头、状态/协议/地区/服务器芯片和 URI 复制动作；桌面端详情以内容侧栏呈现，不再覆盖节点卡片，移动端保留 Dock 上方抽屉，并纳入浏览器摘要溢出、桌面遮挡和点击反馈验收。
...
- 本地 smoke、API flow、页面截图多视口矩阵验收脚本。
