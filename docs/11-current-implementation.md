...
- 管理后台各模块创建/导入表单已默认收纳为可展开抽屉，抽屉标题补齐统一功能符号，列表、卡片和详情成为模块首屏重点，减少常驻表单压缩内容区。
...
- 管理后台模块快速定位条已补齐功能符号、数量和新增徽标，列表入口直接展示当前规模，创建入口统一展示 `+`，浏览器 QA 会校验每个定位按钮都有符号与徽标且无横向溢出。
...
- 浏览器验收会确认模块快速定位条的符号顺序、徽标数量和创建入口展开能力。
