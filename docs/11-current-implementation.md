...
- 管理后台概览页已提供真实测试闭环看板，按来源、节点池、虚拟网关、团队 Token 和访问策略展示准备状态，并给出下一步模块入口；每个闭环步骤都有符号化状态单元，浏览器 QA 会校验符号顺序和无视觉溢出。
...
- 浏览器验收会确认概览页包含 7 个带符号的顶部指标卡、5 个带符号状态单元的真实测试闭环项、1 个下一步动作卡和 4 个带模块符号与实时徽标的快速入口。
