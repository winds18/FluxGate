...
- sing-box 客户端订阅生成。
- sing-box 服务端配置生成骨架。
- active VLESS/Trojan/Shadowsocks/VMess/Hysteria2/TUIC/AnyTLS/ShadowTLS/Naive 上游节点会转换为 sing-box outbound，并通过默认 selector 承接网关出口。
...
- 上游订阅定时同步。
- 上游订阅更多协议格式解析。
- Hysteria 等更多协议 URI 到 sing-box outbound 的转换。
- 策略管理。
- 流量统计采集。
