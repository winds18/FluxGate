...
- 管理后台概览页快速入口卡已补齐实时数量和就绪徽标，接入来源、节点地区、Token 和发布状态能在概览首屏直接扫描；浏览器 QA 会校验 4 个入口徽标都有填充值且无视觉溢出。
...
- 浏览器验收会确认概览页包含 7 个带符号的顶部指标卡、5 个真实测试闭环状态项、1 个下一步动作卡和 4 个带实时徽标的快速入口。
