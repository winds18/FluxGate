...
- JSON 结构化节点可归一化 `skipCertVerify`、`allowInsecure`、`disableSNI`、`clientFingerprint`、`pluginOpts`、`wsOpts`、`wsHeaders`、`grpcServiceName`、`privateKeyPath` 等常见 camelCase 参数，并支持 `tls.enabled`、`tls.serverName`、`tls.utls.fingerprint` 等嵌套 TLS 字段，保留 TLS、插件和传输配置。
