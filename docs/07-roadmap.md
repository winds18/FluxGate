...
- JSON 结构化节点可归一化 `skipCertVerify`、`allowInsecure`、`disableSNI`、`clientFingerprint`、`pluginOpts`、`wsOpts`、`wsHeaders`、`grpcServiceName`、`privateKeyPath`、`publicKey`、`shortId`、`spiderX` 等常见 camelCase 参数，并支持 `tls.enabled`、`tls.serverName`、`tls.utls.fingerprint` 和 `reality.publicKey` 等嵌套字段，保留 TLS、REALITY、插件和传输配置。
