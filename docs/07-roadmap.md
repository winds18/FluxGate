...
- 管理后台概览页已把真实测试闭环看板升级为初始化向导，可直接查看来源、节点池、虚拟网关、团队 Token 和访问策略是否齐备；向导会展示可测进度、当前建议动作、模块入口和每步提示，概览快速定位条也同步为“初始化”标签和向导符号；每个闭环步骤都有序号、符号化状态单元和就绪/待补徽标，并纳入浏览器进度、当前动作、提示文案、定位条标签/符号、状态徽标和溢出验收。
