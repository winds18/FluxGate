...
- SQLite migration。
- 嵌入式管理后台只读仪表盘。
- 管理后台已切换为模块化控制台布局，包含桌面侧边导航、7 模块移动底部 Dock、概览/接入/节点/身份/策略/流量/运维视图，并纳入浏览器导航和溢出验收。
- 管理后台创建/导入表单默认收纳为模块抽屉，减少首屏视觉负担。
- 管理后台概览页已加入真实测试闭环看板，可直接查看来源、节点池、虚拟网关、团队 Token 和访问策略是否齐备。
- 管理后台模块标题区已加入随视图变化的上下文摘要条，帮助在进入每个模块前先确认关键状态。
- 管理后台模块标题区已加入随视图变化的快速定位条，可直接跳转到模块内列表、详情和创建抽屉；创建入口会自动展开对应表单。
- 管理后台模块快速定位条已补齐数量和新增徽标，让入口既能导航也能展示当前模块规模，并纳入浏览器徽标数量和溢出验收。
- 管理后台各模块列表/详情面板标题已统一展示数量或就绪状态徽标，减少进入模块后的二次扫读成本。
- 管理后台团队和成员列表已改为卡片式信息层级，团队归属、邮箱、备注和状态分层展示，并纳入视觉溢出验收。
- 管理后台上游来源列表已改为卡片式信息层级，保留编辑、刷新和同步命名动作，并纳入视觉溢出验收。
- 管理后台虚拟网关列表已改为卡片式信息层级，监听信息、策略、状态和标签选择器分层展示，并纳入视觉溢出验收。
- 管理后台策略列表已改为卡片式信息层级，作用域、标签限制、允许虚拟网关、最大节点数和状态分层展示，并纳入视觉溢出验收。
- 管理后台 Token 订阅地址已拆成默认、Clash/Mihomo 和 sing-box 的可复制订阅卡，长 URL 省略在卡片内部，避免挤压操作按钮。
- 管理后台 Token 订阅卡已补齐“打开”入口，可直接预览或下载三类订阅内容，并纳入浏览器数量和溢出验收。
- 管理后台 Token 订阅卡已补齐通用、Mihomo 和 sing-box 类型徽标，降低真实分发时选错客户端格式的概率，并纳入浏览器数量和溢出验收。
...
- 管理后台 Token 续期和加额操作已补齐可见字段标签与单位，减少真实测试时的输入歧义。
- 管理后台流量模块 Token 用量和上游出口摘要已改为卡片式信息层级，额度进度、今日/本月/累计用量和出口上传下载分层展示，并纳入视觉溢出验收。
- 管理后台运维模块已改为配置操作卡和结构化结果卡，配置检查结果可分层展示 Hash、入站、出口、上游和用户数量，并纳入浏览器检查与视觉溢出验收。
- 管理后台节点池已加入地区/节点/协议/来源/标签/服务器搜索和一键清空，便于在大量节点和地区中快速定位目标节点。
- 管理后台节点详情已加入 URI 复制动作，并纳入浏览器点击反馈验收。
