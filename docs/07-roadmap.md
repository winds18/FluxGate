...
- V2Ray/Xray JSON SOCKS outbound 可保留 `udp`、`udpEnabled`、`udp_enabled`、`udp_relay` 和 `udp-relay` 标志，结构化订阅导入后会继续进入 SOCKS UDP 转换链路。
...
- V2Ray/Xray JSON Shadowsocks outbound 可保留 `network` 或 `protocol` 标志，结构化订阅导入后会继续进入 Shadowsocks 转换链路。
