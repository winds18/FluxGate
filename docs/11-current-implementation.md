...
- 管理后台概览页已把真实测试闭环升级为初始化向导，按来源、节点池、虚拟网关、团队 Token 和访问策略展示准备状态，同时提供可测进度、当前建议动作、可点击模块入口和每步提示文案；概览快速定位条同步展示“初始化”和向导符号，避免旧术语干扰；每个闭环步骤都有序号、符号化状态单元和就绪/待补徽标，浏览器 QA 会校验进度、当前动作、5 条提示、定位条标签/符号、序号、符号、状态徽标和无视觉溢出。
...
- 管理后台运维模块已把交付收口、配置检查、发布、回滚和重启拆为带符号和状态胶囊的操作卡；配置/收口结果使用带结果符号、状态胶囊和结构化字段的结果卡展示 Hash、入站、出口、上游、用户和下一步动作等关键字段，浏览器 QA 会点击检查配置和检查收口并验证操作卡、按钮符号、结果卡和无视觉溢出。
...
- 节点详情已从节点卡片内部改为右侧详情抽屉，先展示协议符号、节点名、来源、状态、协议、地区和服务器地址，再展开字段网格；长 URI 会在抽屉内换行收束，复制动作保留，复制后按钮显示“已复制”并同步顶部状态，抽屉可通过关闭按钮收起；浏览器 QA 会实际展开节点详情抽屉，校验摘要芯片和 URI 无溢出并点击复制入口。
- 管理员引导账号。
- PBKDF2-SHA256 密码哈希。
- 管理员签名 Cookie 会话。
- 管理 API 登录保护。
...
- 浏览器验收会确认 7 个创建/导入表单抽屉默认收纳、标题符号顺序正确、展开后具备取消动作且标题区无视觉溢出。
...
- 有节点数据时，浏览器验收会确认节点池“详情”入口可打开单节点详情抽屉。
...
- 浏览器验收会确认运维模块 5 个带符号和状态胶囊的操作卡可见，并点击“检查配置”和“检查收口”验证结果卡符号、状态胶囊、结构化字段和无视觉溢出。
- 有 Token 数据时，浏览器验收会确认每行都展示带可见标签和单位的自定义续期天数、追加额度 MiB 输入控件、状态控制动作区和带格式画像、地址来源的可复制订阅卡，实际点击复制按钮确认反馈与符号按钮恢复，并点击订阅探测按钮确认“可访问”和最近探测结果可见；同时会点击重置订阅入口，确认站内危险操作框标题、双按钮结构、取消反馈和关闭状态，并确认创建/重置后的订阅结果卡具备符号标题和格式数量胶囊，Token 动作区可见且无溢出或横向滚动。
- 有节点数据时，浏览器验收会展开节点编辑表单确认保存按钮符号化，再打开节点详情抽屉、确认 URI 不横向溢出，并实际点击复制 URI 按钮确认反馈。
...
- 更多真实订阅样本和特殊协议兼容性验证。
- 手把手使用说明需要随后续功能持续更新。
- 初始化向导需要继续随真实测试流程补充更细的场景提示和一键到位动作。
