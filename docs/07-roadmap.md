...
- JSON 结构化节点可归一化 `skipCertVerify`、`allowInsecure`、`allow_insecure`、`allow-insecure`、`disableSNI`、`clientFingerprint`、`networkType`、`transportType`、`pluginOpts`、`wsOpts`、`wsHeaders`、`grpcServiceName`、`privateKeyPath`、`publicKey`、`shortId`、`shortIds`、`spiderX`、`serverNames`、`server_names`、`server-name` 等常见 camelCase/snake_case/kebab-case 参数，并支持 `tls.enabled`、`tls.serverName`、`tls.serverNames`、`tls.server_names`、`tls.server-name`、`tls.utls.fingerprint`、`reality.publicKey` 和 `reality.serverNames` 等嵌套字段，保留 TLS、REALITY、插件和传输配置。
