...
- 管理后台已参考 zashboard 的模块化控制台信息架构重构为桌面侧边导航、移动底部 Dock、概览/接入/节点/身份/策略/流量/运维模块视图；移动 Dock 已补齐 7 个模块入口，写操作表单收敛到对应模块内，避免单页堆叠导致扫描困难。
- 管理后台各模块创建/导入表单已默认收纳为可展开抽屉，列表、卡片和详情成为模块首屏重点，减少常驻表单压缩内容区。
- 管理后台概览页已提供真实测试闭环看板，按来源、节点池、虚拟网关、团队 Token 和访问策略展示准备状态，并给出下一步模块入口。
- 管理后台模块标题区已加入随当前视图变化的数据摘要条，进入接入、节点、身份、策略、流量和运维模块时能先看到来源数、地区数、有效 Token、策略限制和流量样本等关键上下文。
- 管理后台模块标题区已加入当前模块快速定位条，可在接入、节点、身份、策略、流量和运维视图内直接跳转到列表、详情或创建抽屉；点击创建入口会自动展开对应表单，减少长页面反复滚动。
- 管理后台模块快速定位条已补齐数量和新增徽标，列表入口直接展示当前规模，创建入口统一展示 `+`，浏览器 QA 会校验每个定位按钮都有徽标且无横向溢出。
- 管理后台各模块列表/详情面板标题已统一为标题加数量徽标，来源、节点池、虚拟节点、团队、成员、Token、策略、流量和配置发布区都能在面板头部直接扫到当前规模或就绪状态。
- 管理后台团队和成员列表已改为卡片列表，名称、团队归属、邮箱、备注和状态分层展示；原有编辑动作保留，浏览器 QA 会验证身份卡片数量、编辑入口和无视觉溢出。
- 管理后台上游来源已改为卡片列表，来源名称、类型、URL、前缀、默认标签、刷新间隔、上次同步和异常状态分层展示；原有编辑、刷新和同步命名动作保留，浏览器 QA 会验证来源卡片数量、编辑入口和卡片无视觉溢出。
- 管理后台虚拟网关列表已改为卡片列表，名称、监听协议/端口、策略、状态和标签选择器分层展示；编辑动作保留在卡片内，浏览器 QA 会验证虚拟网关卡片数量、编辑入口和无视觉溢出。
- 管理后台策略列表已改为卡片列表，策略名称、作用域、标签限制、允许虚拟网关、最大节点数和状态分层展示；编辑动作保留在卡片内，浏览器 QA 会验证策略卡片数量、编辑入口和无视觉溢出。
- 管理后台 Token 卡片会把默认、Clash/Mihomo 和 sing-box 订阅地址拆成可复制订阅卡，长 URL 只在卡内省略显示，浏览器 QA 会校验订阅卡数量和无视觉溢出。
- 管理后台 Token 订阅卡除复制外已提供“打开”入口，可直接在新标签页预览或下载默认、Clash/Mihomo 和 sing-box 订阅内容，浏览器 QA 会校验打开入口数量和无视觉溢出。
...
- 管理后台 Token 续期和加额操作已补齐可见字段标签与单位，避免只能靠默认数字猜测输入含义；浏览器 QA 会校验每张 Token 卡都有两组带标签的操作字段。
- 管理后台运维模块已把配置检查、发布、回滚和重启拆为操作卡；配置结果使用结构化结果卡展示 Hash、入站、出口、上游和用户等关键字段，浏览器 QA 会点击检查配置并验证操作卡和结果卡无视觉溢出。
- 管理后台已做基础视觉统一：统一顶部栏、卡片、表单、按钮、表格、状态徽标和响应式栅格，输入控件高度保持一致；节点卡片长标题、来源名和标签会限制在父卡片内换行/截断，并通过浏览器 QA 检测桌面和移动视图是否发生视觉溢出。
- 节点池支持在地区聚合页和地区节点卡片页按地区、节点名、协议、来源、标签或服务器搜索，并可一键清空筛选；浏览器 QA 会验证搜索可见、筛选有效、清空可恢复且不产生横向溢出。
- 节点详情面板已加入 URI 复制动作，复制后按钮显示“已复制”并同步顶部状态；浏览器 QA 会实际展开节点详情并点击复制入口。
- 管理员引导账号。
- PBKDF2-SHA256 密码哈希。
- 管理员签名 Cookie 会话。
- 管理 API 登录保护。
...
- 管理后台流量模块的 Token 用量和上游出口摘要已改为卡片式信息层级，Token 用量卡展示额度使用率进度条，并按接近超额和已超额状态变色；浏览器 QA 会验证流量卡片数量、额度条和无视觉溢出。
...
- 浏览器验收会确认 7 个创建/导入表单抽屉默认收纳。
- 浏览器验收会确认每个模块标题区至少展示 3 个上下文摘要项。
- 浏览器验收会确认 9 个模块面板数量/状态徽标都已渲染且非空。
- 浏览器验收会确认概览页真实测试闭环看板包含 5 个状态项和 1 个下一步入口。
- 无白屏。
- 无明显遮挡。
...
- 有 Token 流量数据时，浏览器验收会确认流量卡片、额度使用率进度条和出口摘要卡片无视觉溢出。
- 浏览器验收会确认运维模块 4 个操作卡可见，并点击“检查配置”验证结果卡结构和无视觉溢出。
- 有 Token 数据时，浏览器验收会确认每行都展示带可见标签和单位的自定义续期天数、追加额度 MiB 输入控件和可复制订阅卡，实际点击复制按钮确认反馈，并确认创建/重置后的订阅结果卡可见且无溢出。
- 有节点数据时，浏览器验收会展开节点详情、确认 URI 不横向溢出，并实际点击复制 URI 按钮确认反馈。
