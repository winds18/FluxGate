...
- 节点详情已从节点卡片内部改为详情面板：桌面端作为地区节点列表右侧的内容侧栏，移动端保留 Dock 上方抽屉，先展示协议符号、节点名、来源、状态、协议、地区和服务器地址，再展开字段网格；长 URI 会在面板内换行收束，复制动作保留，复制后按钮显示“已复制”并同步顶部状态，面板可通过关闭按钮收起；浏览器 QA 会实际展开节点详情，校验摘要芯片、URI 无溢出、桌面不覆盖节点卡片并点击复制入口。
- 管理员引导账号。
- PBKDF2-SHA256 密码哈希。
- 管理员签名 Cookie 会话。
- 管理 API 登录保护。
...
- 浏览器验收会确认 7 个创建/导入表单抽屉默认收纳、标题符号顺序正确、默认宽度覆盖模块操作区、展开后具备取消动作且标题区无视觉溢出。
