...
- JSON 结构化节点支持常见字段别名，例如 `protocol`/`proto`/`scheme`/`nodeType`/`proxyType`/`proxyProtocol`、`host`/`hostname`/`address`/`serverAddress`/`serverHost`/`remoteHost`/`add`、`server_port`/`serverPort`/`server-port`/`remotePort`、`method`/`encryptMethod`/`encrypt-method`、`pass`/`passwd`/`pwd`、`id`/`user-id` 和 `displayName`/`label`/`title`/`remarks`。
