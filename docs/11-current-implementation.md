...
- 管理后台流量模块的 Token 用量和上游出口摘要已改为卡片式信息层级，Token 用量卡增加状态、成员、今日和本月摘要芯片，上游出口卡增加来源、上传、下载和总量摘要芯片；Token 用量卡展示额度使用率进度条，并按接近超额和已超额状态变色；浏览器 QA 会验证流量卡片数量、摘要芯片、额度条和无视觉溢出。
