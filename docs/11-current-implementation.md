...
- sing-box JSON `outbounds` 和 `endpoints` 支持数组、单个对象和按名称分组的对象映射；对象映射缺少 `tag`/`name` 时会使用映射键作为节点名。
...
- Clash YAML、sing-box JSON 和 URI 导入链路会保留 Hysteria2 `up_mbps`、`down_mbps`、`insecure`、`disable_sni` 和 `alpn` 参数，并同步为 sing-box outbound。
