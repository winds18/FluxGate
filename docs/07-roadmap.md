...
- JSON 结构化节点可归一化 `skipCertVerify`、`allowInsecure`、`disableSNI`、`clientFingerprint`、`pluginOpts`、`wsOpts`、`wsHeaders`、`grpcServiceName`、`privateKeyPath`、`publicKey`、`shortId`、`shortIds`、`spiderX`、`serverNames` 等常见 camelCase 参数，并支持 `tls.enabled`、`tls.serverName`、`tls.serverNames`、`tls.utls.fingerprint`、`reality.publicKey` 和 `reality.serverNames` 等嵌套字段，保留 TLS、REALITY、插件和传输配置。
