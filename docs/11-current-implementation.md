...
- 管理后台已参考 zashboard 的模块化控制台信息架构重构为桌面侧边导航、移动底部 Dock、概览/接入/节点/身份/策略/流量/运维模块视图；写操作表单收敛到对应模块内，避免单页堆叠导致扫描困难。
...
- 浏览器验收会在 390px 移动宽度确认底部 Dock 可见、桌面侧边栏隐藏，且概览和节点视图不产生横向溢出。
