...
这个脚本会创建缺失的默认虚拟节点和默认可用策略；如果已有可用 Token 已过期或额度用尽，会先自动续期或补额度；随后校验并发布 sing-box 配置。部署配置里有远程主机信息时，还会重启远程 sing-box 容器让配置生效。
...
4. 复制创建结果里返回的三类订阅地址。地址会优先按你当前访问后台的域名、IP 和端口生成，例如从 `http://192.168.66.10:18080` 登录时会生成同域名的 `/sub/...` 地址：
   - 默认订阅
   - Clash/Mihomo 订阅
   - sing-box 订阅
...
重置订阅地址会让旧订阅地址失效，适合旧 Token 没有可恢复订阅地址、或订阅地址已经泄露的情况。旧 Token 如果历史上只保存了 hash，后台可以根据当前访问域名生成地址前缀，但无法反推 `/sub/` 后面的密钥；这种情况需要重置一次。撤销、过期、超额的 Token 不会进入新生成的 sing-box 配置。
...
5. 展开节点配置时，`server` 应该是当前访问订阅的域名或局域网 IP，端口应是虚拟节点端口，例如 `8443`；如果看到 `gateway.example.com` 说明还在使用占位配置，需要重新拉取新版订阅或检查反代 Host。
6. 连接后访问测试网站。
7. 回到管理后台查看 Token 流量摘要和额度进度。
