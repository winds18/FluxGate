...
- JSON 结构化节点会归一化常见 camelCase 参数，例如 `skipCertVerify`、`allowInsecure`、`disableSNI`、`clientFingerprint`、`pluginOpts`、`wsOpts`、`wsHeaders`、`grpcServiceName`、`privateKeyPath` 等，并支持 `tls.enabled`、`tls.serverName`、`tls.utls.fingerprint` 等嵌套 TLS 字段，避免导入后丢失 TLS、插件和传输配置。
