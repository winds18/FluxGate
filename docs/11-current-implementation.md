...
- 管理后台 UI 已重构为 v3「安静控制台」设计系统：单一 `styles.css` 样式层（设计令牌 + 共享组件），替代历史上 `styles.css` + `calm-ops.css` 双层叠加；统一中性浅灰背景、白色发丝边卡片、单一主色、点状态语义和扁平字段排版。
- 信息架构保持七个模块视图（概览/接入/节点/订阅/策略/流量/运维），桌面侧边导航 + 移动底部 Dock；工作区头部收敛为「标题 + 描述 + 主操作 + 刷新 + 会话状态」单行，历史的洞察条、上下文芯片条和模块定位条已删除，消除同屏计数重复。
- 各模块保留「工作台统计条 + 实体卡片列表」结构：工作台改为扁平数字统计与分布行，实体卡片字段改为 label/value 扁平网格（不再逐字段包盒），卡片操作收敛为右上角图标按钮（移动端恢复全宽触控）。
- 创建/导入表单仍为右侧侧滑表单页（dialog 语义 + 背景遮罩 + Escape 收起 + 草稿感知），节点详情仍为右侧抽屉；移动端两者均避开底部 Dock 安全区。
- 列表接口在空库时返回 JSON null 的情况已在前端统一归一为空数组（`asList`），修复了概览页空库时的渲染崩溃。
- 静态 UI 契约 `scripts/qa/ui-static.sh` 重写为行为/结构断言（含旧叠层必须不存在的负向检查），不再锁定像素值；浏览器验收 `scripts/qa/browser-login.sh` 重写为精简行为用例（登录、七模块切换、表单抽屉、节点详情、移动 Dock、无脚本错误）。
- 管理员引导账号。
- PBKDF2-SHA256 密码哈希。
- 管理员签名 Cookie 会话。
- 管理 API 登录保护。
